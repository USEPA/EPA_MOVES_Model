...
-- Emission factors in grams per vehicle by equipment type
-- 
...
-- Get population by equipment type
...
drop table if exists EmissionFactors_per_Vehicle_by_Equipment;
create table EmissionFactors_per_Vehicle_by_Equipment
select
	b1.MOVESRunID,
	b1.yearID,
	b1.monthID,
...
set @sqlstmt := if( @exist = 0, 'select ''INFO: index102 does not exist.''', 'drop index index100 on movesoutput');
PREPARE stmt FROM @sqlstmt;																						
EXECUTE stmt;
