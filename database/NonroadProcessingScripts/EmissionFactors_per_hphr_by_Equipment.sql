...
join horsepower b2
	on ((b1.MOVESRunID=b2.MOVESRunID) AND
		...
		(b1.hpID=b2.hpID OR b1.hpID IS NULL AND b2.hpID IS NULL));
...
set b1.loadFactor = b3.loadFactor,
 ...
 where ((b1.MOVESRunID=b3.MOVESRunID) AND
		...
		(b1.hpID=b3.hpID OR b1.hpID IS NULL AND b3.hpID IS NULL));
...
-- Get horsepower-hours by equipment type
...
drop table if exists EmissionFactors_per_hphr_by_Equipment;
create table EmissionFactors_per_hphr_by_Equipment
select
	b1.MOVESRunID,
	b1.yearID,
	b1.monthID,
