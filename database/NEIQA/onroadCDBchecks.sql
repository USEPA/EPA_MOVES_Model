...
set @version = CONCAT('db', MID("##defaultdb##", 8, 8));

-- Coordinate this message with the Done statement at the end of this file.
select '  .. onroadCDBchecks.sql',curTime(), database();
...
BEGIN
  DECLARE done INT DEFAULT FALSE;
  declare Czone, CftID, ComID, CBEGMY, CENDMY int;  -- C is for current (row of data)
  declare Lzone, LftID, LomID, LBEGMY, LENDMY int;  -- L is for last     "   "  "
  declare rows_processed,
          rows_skipped   int default 0;
  declare reason char(40);
  declare sameSet int;
  ...
  read_loop: LOOP
    FETCH curHAD INTO Czone, CftID, ComID, CBEGMY, CENDMY;
    IF done THEN LEAVE read_loop; END IF;
	...
	then
	  Set reason = 'CbegModelYearId>CendModelYearId,';
	  INSERT INTO qa_checks_had values ( Czone, CftID, ComID, LENDMY, CBEGMY, CENDMY, reason );
	else
	  if Czone=Lzone AND CftID=LftID AND ComID=LomID
	  then  -- in set:
...
--       check no. 2302 -- check that the opModeFraction sums to 1 by model year range for each fuel type
INSERT INTO QA_Checks_Log values ( 2302, 'OK', @hVersion, curDate(), curTime() );
Drop table if exists tempA;
Create table tempA
select   modelYearID,fueltypeid,
         sum(opModeFraction) as s,
         count(*)            as cou
From     hotellingactivitydistribution
JOIN     ##defaultdb##.modelyear ON (modelYearID BETWEEN beginModelYearID AND endModelYearID)
group by modelYearID,fueltypeid;
...
Select   "hotellingActivityDistribution" as tableName,
         2302                            as checkNumber,
        'distribution<>1.0'              as testDescription,
         CONCAT('Model year range covering ', modelYearID, ' sums to ', s)               as testValue,
         fuelTypeId                      as fuelTypeID,
         cou                             as count
from     tempA
where    s<0.99999
   or    s>1.00001
...
--       check no. 2305 -- check for unknown fuelTypeID
INSERT INTO QA_Checks_Log values ( 2305, 'OK', @hVersion, curDate(), curTime() );
Drop table if exists tempA;
Create table tempA
Select   fuelTypeID,
         'no '           as aMatch,
...
Update tempA as a 
inner join ##defaultdb##.fueltype as c on a.fuelTypeID = c.fuelTypeID
set aMatch='yes';								
...
from     tempA
where    aMatch <> 'yes';
Delete from CDB_Checks where checkNumber=2305 and count=0;
...
select '  .. onroadCDBchecks.sql Done',curTime(),database();
