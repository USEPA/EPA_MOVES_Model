 ...
 * @version 	2020-08-13
**/
public class ProfileWeightScriptNR extends JDialog implements ActionListener {
	...
	**/
	public ProfileWeightScriptNR(JFrame parent) {
		super(parent, "Nonroad Speciation Profile Script");
		frame = parent;
	...
	public JPanel createAndArrangeControls() {
		...
		try {
			...
			doc.insertString(doc.getLength(),
				"This script matches speciation profiles used to speciate nonroad emissions"
				+ " to MOVES output for residual total organic gases (NonHAPTOG) and particulate matter (PM)."
				+ " This tool is designed for users who are interested in air quality modeling or"
				+ " applying chemical mechanisms."
				+ "\r\n\r\n"
				+ "The script runs against a MOVES output database with output for the two pollutants"
				+ " for a single calendar year and a single county. The MOVES runs must have output by"
				+ " SCC, fuel type, fuel subtype, and engine tech. It will"
				+ " write the profile weights to a different database which can be selected below."
				+ " For more detail on how to perform the MOVES runs and how the profile tables"
				...
				+ " output tables including speciation profiles as the \"New Database\". If the"
				+ " database doesn't exist, it will be created. Use the"
				+ " \"Run Profile Script\" button to execute the script file."
				+ "\r\n\r\n",
				normal);
			...
			createRunSpecsButton.setText("Run Profile Script");
	...
	**/
	...
	void handleConvertButton() {
		try {
			...
			try {
				DatabaseUtilities.executeProfileWeightScriptNR(outputDatabase,newDatabase,defaultDatabase,messages);
			...
			if(messagesHasError) {
				...
				**/
				JOptionPane.showMessageDialog(null,
						...
						"Speciation Profile Script", JOptionPane.ERROR_MESSAGE);
			} else {
				/** @nonissue **/
				JOptionPane.showMessageDialog(null,
						"Speciation profile script has run successfully.",
						"Speciation Profile Script", JOptionPane.INFORMATION_MESSAGE);
			}
		} catch(Exception e) {
			/**
	...
	void handleOpenHelpButton() {
		try {
			File file = new File("database/ProfileWeightScripts/NonroadSpeciationInstructions.pdf");
			if(!file.exists()) {
