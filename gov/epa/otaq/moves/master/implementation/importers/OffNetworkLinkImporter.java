...
/**
 * MOVES OffNetworkLink Importer.  Imports data in MOVES format (as opposed to
...
public class OffNetworkLinkImporter extends ImporterBase {
	...
	**/
	String primaryTableName = "offNetworkLink";
	...
	**/
	static String[] dataTableDescriptor = {
		BasicDataHandler.BEGIN_TABLE, "OffNetworkLink",
		"zoneID", "Zone", ImporterManager.FILTER_ZONE,
		"sourceTypeID", "SourceUseType", ImporterManager.FILTER_SOURCE,
		"vehiclePopulation", "", ImporterManager.FILTER_NON_NEGATIVE,
		"startFraction", "", ImporterManager.FILTER_NON_NEGATIVE,
		"extendedIdleFraction", "", ImporterManager.FILTER_NON_NEGATIVE,
		"parkedVehicleFraction", "", ImporterManager.FILTER_NON_NEGATIVE
	};
	...
	/** Constructor **/
	public OffNetworkLinkImporter() {
		super("Off-Network", // common name
				"offnetworklink", // XML node name
				new String[] { "OffNetworkLink" } // required tables
				);
	...
	**/
	public RunSpecSectionStatus getProjectDataStatus(Connection db) 
			throws Exception {
		if(db == null || !manager.getFilterValuesSet(ImporterManager.FILTER_ROAD_TYPE)
				.contains(offnetworkRoadTypeID)) {
			// If there is no database being checked
			// or if the RunSpec doesn't use the offnetwork information...
