...
/**
 * MOVES RoadType.rampFraction Importer.
 * 
 * @author		Wesley Faler
 * @version		2013-04-17
**/
public class RampFractionImporter extends ImporterBase {
	/** Data handler for this importer **/
	BasicDataHandler basicDataHandler;
	/** Part object for the RoadType table **/
	...
	**/
	String primaryTableName = "roadType";
	...
	**/
	static String[] dataTableDescriptor = {
		BasicDataHandler.BEGIN_TABLE, "RoadType",
		"roadTypeID", "RoadTypeHwy", ImporterManager.FILTER_ROAD_TYPE_HWY,
		"rampFraction", "", ImporterManager.FILTER_0_TO_1_FRACTION
	};
	...
	/** Constructor **/
	public RampFractionImporter() {
		super("Ramp Fraction", // common name
				"rampfraction", // XML node name
				new String[] { "RoadType" } // required tables
				);
	...
	**/
	public RunSpecSectionStatus getCountyDataStatus(Connection db) 
			throws Exception {
		for(Iterator i=manager.runSpec.roadTypes.iterator();i.hasNext();) {
		...
		if(db == null) {
			System.out.println("RampFractionImporter skipped due to lack of RunSpec highways.");
			return null;
		}
		return new RunSpecSectionStatus(RunSpecSectionStatus.OK);
	}
