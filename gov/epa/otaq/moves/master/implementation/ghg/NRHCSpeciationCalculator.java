...
/**
 * Perform HC Speciation calculations for Nonroad
 * @author		Wesley Faler
 * @version		2015-03-11
**/
public class NRHCSpeciationCalculator extends EmissionCalculator implements EmissionCalculatorExternal {
	/**
	 * @algorithm
	...
	**/
	...
	class HCEntry {
		...
		HCEntry(int outputPolProcessIDToUse) {
			...
			if(output != null) {
				...
				TreeSet<Integer> inputIDs = PollutantProcessAssociation.chainedToNR(outputPolProcessID);
				if(inputIDs != null && inputIDs.size() > 0) {
					for(Iterator<Integer> iterInputIDs = inputIDs.iterator(); iterInputIDs.hasNext();) {
	 ...
	 * chaining.
	**/
	public NRHCSpeciationCalculator() {
		loadEntries();
	...
	private void loadEntries() {
		//Logger.log(LogMessageCategory.INFO,"NRHCSpeciationCalc loadEntries...");
		entries.clear();
		String sql = "";
		...
		try {
			sql = "select polProcessID"
					...
					+ " and isAffectedByNonroad=1";
			db = DatabaseConnectionManager.checkOutConnection(MOVESDatabaseType.EXECUTION);
			query.open(db,sql);
			while(query.rs.next()) {
				int polProcessID = query.rs.getInt(1);
				HCEntry a = new HCEntry(query.rs.getInt(1));
				if(a.output != null && a.inputs != null) {
					//Logger.log(LogMessageCategory.INFO,"NRHCSpeciationCalc loadEntries accepted " + polProcessID);
					entries.add(a);
				} else {
					//Logger.log(LogMessageCategory.INFO,"NRHCSpeciationCalc loadEntries rejected " + polProcessID);
				}
			}
			query.rs.close();
		} catch(Exception e) {
			//Logger.log(LogMessageCategory.INFO,"NRHCSpeciationCalc loadEntries failed: " + e.toString());
		...
		//Logger.log(LogMessageCategory.INFO,"NRHCSpeciationCalc loadEntries done.");
	}
	...
	**/
	public void subscribeToMe(MasterLoop targetLoop) {
		Logger.log(LogMessageCategory.INFO,"NRHCSpeciationCalc subscribing...");
		...
		for(Iterator<EmissionCalculator> i=calculators.iterator();i.hasNext();) {
			...
			if(c != this) {
				Logger.log(LogMessageCategory.INFO,"NRHCSpeciationCalc chained to " + c.getClass().getName());
	...
	**/
	public SQLForWorker doExecute(MasterLoopContext context) {
		...
		/*
		Logger.log(LogMessageCategory.INFO,"##hcPolProcessIDs##"+hcPolProcessIDs);
		Logger.log(LogMessageCategory.INFO,"##nmhcProcessIDs##"+nmhcProcessIDs);
		Logger.log(LogMessageCategory.INFO,"##nmogProcessIDs##"+nmogProcessIDs);
		Logger.log(LogMessageCategory.INFO,"##vocProcessIDs##"+vocProcessIDs);
		Logger.log(LogMessageCategory.INFO,"##togProcessIDs##"+togProcessIDs);
		Logger.log(LogMessageCategory.INFO,"##hcProcessIDs##"+hcProcessIDs);
		*/
		boolean isOK = readAndHandleScriptedCalculations(context,replacements,
				"database/NRHCSpeciationCalculator.sql",enabledSectionNames,
				sqlForWorker);
