	...
	**/
	private boolean launchCore(RunSpec runSpec, String runSpecFileName, Integer numberOfDONEFiles)
		...
		try {
			...
			if (runSpec.onRoadVehicleSelections.size() == 0 && runSpec.offRoadVehicleSelections.size() == 0) {
				/**
				 * @issue RunSpec seemingly does not contain any vehicle selections. Normally, a user cannot launch a run without these; however, if they created the RunSpec with an older version of MOVES and are running on the command line, they will bypass this check and have this issue.
