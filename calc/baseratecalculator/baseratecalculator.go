	...
	for {
		...
		for _, fb := range b.FuelBlocks {
			...
			if zmh != nil && ft != nil && ta != nil {
				...
				for _, br := range fb.OpMode.BaseRates {
					factor := ta.generalTempAdjust(fb.Key.ProcessID, fb.Key.PollutantID, fb.Key.FuelTypeID, fb.Key.SourceTypeID, fb.Key.RegClassID, fb.Key.ModelYearID, k, zmh.temperature, zmh.heatIndex, br.MeanBaseRate)
