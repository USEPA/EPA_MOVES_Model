...
c-----------------------------------------------------------------------
c
c   This routine does the processing for the state records for a state
...
c      06/07/04 --dfk--  added evap processing
...
c      08/01/05 -cimulus- pass zeroems to wrtdat for the zero record
c      04/19/06  --epa-- fixes all Daily Temp/RVP month/season/year.
...
c
      ...
      character*5  fipsta, subcur
      integer*4    jerr, i, j, idxsta, idxref, idxall
      ...
      real*4       hpval, adjems(MXPOL,MXDAYS),adjtime, tank(MXEVTECH)
...
c
      ...
      missems = RMISS
c
c   --- get the index of this state ---
c
      fipsta = regncd(icurec)(1:5)
      idxsta = fndchr( fipsta, 5, statcd, NSTATE )
      if( idxsta .LE. 0 ) then
          ierr = ISKIP
          goto 9999
      endif
...
c
      ...
      hpval = avghpc(icurec)
c
c   --- if the population is zero, just write a zero record and return ---
c
      popsta = popeqp(icurec)
      if( popsta .LE. 0. ) then
         call wrtdat(jerr,fipsta,subcur,asccod,hplev,
...
c       write a missing record, and return ---
c
      if( idxact .LE. 0 ) then
...
c
            if( lbmyfl ) then
               call wrtbmy(jerr,fipsta,subcur,asccod,hplev,
...
c   --- skip over unused technology types ---
c
            if( evtchfrc(idxevtch,i) .LE. 0. ) then
               cycle
            endif
c
c   --- initialize the bmy array ---
...
c
            popbmy = popsta * modfrc(idxyr) * evtchfrc(idxevtch,i)
            actbmy = actadj(idxyr) * popsta * modfrc(idxyr)
...
c              apply to evap by-model-year output
     ...
     &               RMISS,RMISS,RMISS,RMISS,2) ! 2=evap
               if( jerr .NE. ISUCES ) goto 9999
            endif
c
c   --- call routine to add to totals for SI report ---
...
c
      call wrtdat(jerr,fipsta,subcur,asccod,hplev,poptot,acttot,
     &      fulcsm,faclod(idxact),hpval,fracretro,unitsretro,emsday)
      if( jerr .NE. ISUCES ) goto 9999
c
c   --- state processed, return to the calling routine ---
c
      ierr = ISUCES
      goto 9999
c
