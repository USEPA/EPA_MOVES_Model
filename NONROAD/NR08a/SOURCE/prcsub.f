c**** PRCSUB
c
      subroutine prcsub(ierr,emsams,icurec,asccod,growth)
c
c-----------------------------------------------------------------------
c  
c   This routine does the prcessing for the county to subregion allocation
...
c      08/01/05 -cimulus- removed unused poptmp variable
...
c
      ...
      character*5  subcur, fipin
      integer*4    jerr, i, idxasc, idxfip, j
      integer*4    idxact, idxgrw, nyrlif, idxyr, iyr
      integer*4    idxtch, ndays, idxreg
      integer*4    idxunt(MXPOL,MXTECH), idxref, idxall
      real*4       popsub, grwsub, grwtmp
...
c
   ...
   10    continue
      ...
      hpval = avghpc(icurec)
c
c  --- initialize the emissions to zero ---
c
      do 20 i=1,MXPOL
        emsday(i) = 0.
   20 continue
...
c      if not found, write a missing record and return ----
c
      idxasc = fndasc( asccod, ascalo, nalorc )
      if( idxasc .LE. 0 ) goto 7000
c
c  --- loop over the regions, and find the county for this
c      subcounty ---
c
      idxreg = 0
      do 30 i=1,nregin
         if( fipin .EQ. reglst(i)(1:5) ) idxreg = i
   30 continue
      if( idxreg .LE. 0 ) then
         ierr = ISKIP
         goto 9999
      endif
      subcur = reglst(idxreg)(6:10)
      if( subcur .EQ. '     ' ) then
         ierr = ISKIP
         goto 9999
      endif 
...
c
      call alosub(jerr,popsub,grwsub,reglst(idxreg)(6:10),
     &                  icurec,ipopyr(icurec),popeqp(icurec),idxfip,
     &                                               idxasc,luse,growth)
      if( jerr .NE. ISUCES ) goto 9999
c
c  --- if population for this county is zero, just write a zero record 
c      and rerturn to get the next record ---
c
      if( popsub .LE. 0 ) then
...
c
c   --- finf the fuel density, used for fuel consumption calc ---
c
      denful = 1.0
...
c
      call agedist( jerr, popsub, modfrc, ipopyr(icurec),
...
c
            popbmy = popsub * modfrc(idxyr) * tchfrc(idxtch,i)
c
...
c
     ...
     &            tpltmp, stradj(idxyr), popsub, modfrc(idxyr),
     &            ndays, actadj(idxyr) )
            if( jerr .NE. ISUCES ) goto 9999
c
...
c
            actbmy = actadj(idxyr) * popsub * modfrc(idxyr)
     &            * tplful * tchfrc(idxtch,i) * adjtime
            fulbmy = tplful * popsub * actadj(idxyr) * modfrc(idxyr)
...
c 
         poptot = poptot + popsub * modfrc(idxyr)  
         acttot = acttot + actadj(idxyr) * popsub
     &         * modfrc(idxyr) * tplful * adjtime
         strtot = strtot + stradj(idxyr) * popsub
     &         * modfrc(idxyr) * tplful * adjtime
c
c  --------- Evap --------
c
...
c
            popbmy = popsub * modfrc(idxyr) * evtchfrc(idxevtch,i)
            actbmy = actadj(idxyr) * popsub * modfrc(idxyr)
...
c   --- sum population, activity and starts ---
c 
         evpoptot = evpoptot + popsub * modfrc(idxyr)
         evacttot = evacttot + actadj(idxyr) * popsub
     &         * modfrc(idxyr) * tplful * adjtime
         evstrtot = evstrtot + stradj(idxyr) * popsub
     &         * modfrc(idxyr) * tplful * adjtime
c
c   --- next model year ---
c
