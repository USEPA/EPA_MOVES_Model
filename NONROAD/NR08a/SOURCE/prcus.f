c**** PRCUS
c
      subroutine prcus(ierr,icurec,asccod,growth)
c
c-----------------------------------------------------------------------
c  
c   This routine does the processing for the national record, producing
c   output which is the estimates for the entire nation.  No allocation
c   is performed.
...
c-----------------------------------------------------------------------
c
c      05/27/98  --gwilson-- original developnent
...
c      08/31/01 -4 charvey/bgiannelli remove unused declarations.
c      05/13/04 --dfk-- added evap processing
c      10/11/01 rgiannelli force getgrw even in base year
c      06/15/04 --dfk-- added optional daily temperature emission adjustments
...
c
      ...
      character*5  fipus, subcur
      integer*4    jerr, i, icurec, j
      integer*4    idxact, idxgrw, nyrlif, idxyr, iyr
      integer*4    idxtch, ndays
      integer*4    idxunt(MXPOL,MXTECH), idxref, idxall
      real*4       popus, grwus, grwtmp
      ...
      real*4       hpval, adjems(MXPOL,MXDAYS),adjtime, hpmid
...
c
      if( popeqp(icurec) .LE. 0. ) then
          call wrtdat(jerr,fipus,subcur,asccod,hplev,0.,0.,
     &            0.,0.,0.,0.,0.,emsday)
...
c-----------------------------------------------------------------------
c
      call daymthf(asccod, fipus, daymthfac, mthf, dayf, ndays)
c
...
c
     ...
     &         'FIPS','SCC','HP range',
     &         fipus,asccod,hprang(1,icurec),hprang(2,icurec)
         call chkwrn(jerr,IDXWAC)
         if( jerr .NE. ISUCES ) goto 9999
         do j = 1, MXPOL
            if(j.lt.IDXDIU.or.j.gt.IDXRLS) emsday(j) = RMISS
         end do
         call wrtdat(jerr,fipus,subcur,asccod,hplev,RMISS,RMISS,
...
c
     ...
     &      ipopyr(icurec)+1, fipus, indgrx(idxgrw) )
      if( jerr .NE. ISUCES ) goto 9999
      grwus = grwtmp
c
...
c
      call agedist( jerr, popus, modfrc, ipopyr(icurec),
     &      igryr, nyrlif, yryrfrcscrp, fipus, indgrx(idxgrw) )
      if( jerr .NE. ISUCES ) goto 9999
c
...
c
            popbmy = popus * modfrc(idxyr) * tchfrc(idxtch,i)
c
...
c
     ...
     &            tpltmp, stradj(idxyr), popus, modfrc(idxyr),
     &            ndays, actadj(idxyr) )
            if( jerr .NE. ISUCES ) goto 9999
c
c   --- calculate the contribution from this model year and tech type ---
c
            actbmy = actadj(idxyr) * popus * modfrc(idxyr)
     &            * tplful * tchfrc(idxtch,i) * adjtime
            fulbmy = tplful * popus * actadj(idxyr) * modfrc(idxyr)
...
c
            if( lbmyfl ) then
               call wrtbmy(jerr,fipus,subcur,asccod,hplev,
...
c 
         poptot = poptot + popus * modfrc(idxyr)
         acttot = acttot + actadj(idxyr) * popus
     &         * modfrc(idxyr) * tplful * adjtime
         strtot = strtot + stradj(idxyr) * popus
     &         * modfrc(idxyr) * tplful * adjtime
c
c   --------- Evap --------
c
...
c
     ...
     &            sre10fac(i), vnte10fac(i), fipus )
            if( jerr .NE. ISUCES ) goto 9999
c
c   --- calculate the contribution from this tech type ---
c
            popbmy = popus * modfrc(idxyr) * evtchfrc(idxevtch,i)
            actbmy = actadj(idxyr) * popus * modfrc(idxyr)
...
c   --- sum population, activity and starts ---
c 
         evpoptot = evpoptot + popus * modfrc(idxyr)  
         evacttot = evacttot + actadj(idxyr) * popus
     &         * modfrc(idxyr) * tplful * adjtime
         evstrtot = evstrtot + stradj(idxyr) * popus
     &         * modfrc(idxyr) * tplful * adjtime
c
c   --- next model year ---
c
...
c
      call wrtdat(jerr,fipus,subcur,asccod,hplev,poptot,acttot,
