C**** PRCSTA
c
      subroutine prcsta(ierr,emsams,icurec,asccod,idxsta,growth)
c
c-----------------------------------------------------------------------
c
...
c          idxsta  I   index of the state in arrays
...
c      07/29/05 -cimulus- separated main processing to calculate
c                         emissions on state level and then allocate to
c                         county level using the allocation fractions,
c                         instead of calculating emissions for each
c                         county, improving performance significantly
c      07/29/05 -cimulus- simplified find-activity, get-growth, and
c                         model-year-age-distribution handling now that
c                         fndact no longer depends on tech type
c      07/29/05 -cimulus- cleaned up code for consistency, in terms of
c                         whitespace and commenting
c      07/29/05 -cimulus- removed unused rfmode and fulcsm parameters
c                         from clcems call
c      07/29/05 -cimulus- removed redundant thisfc variable
c      07/29/05 -cimulus- changed size of rfmode and tank, which are
...
c
      character*5  fipsta
      real*4       popsta, grwsta
c
c   --- county level ---
c
      integer*4    ibegj, iendj
      integer*4    idxfip
      real*4       emsdaycty(MXPOL)
      real*4       popcty(NCNTY), grwcty(NCNTY)
      real*4       popctyfrac
      real*4       poptotcty
      real*4       acttotcty
      real*4       fulcsmcty
      real*4       fracretrocty
      real*4       unitsretrocty
c
c   --- exhaust tech ---
c
      ...
      real*4       exemsbmy(MXAGYR,MXTECH,MXPOL)
      real*4       expopbmy(MXAGYR,MXTECH)
      real*4       exactbmy(MXAGYR,MXTECH)
      real*4       exfulbmy(MXAGYR,MXTECH)
      real*4       poptot
      real*4       acttot
      real*4       strtot
      ...
      real*4       fracretrobmy ! fraction retrofitted for a single model year's and tech type's population
      real*4       unitsretrobmy ! number of units retrofitted for a single model year's and tech type's population
      real*4       exfracretrobmy(MXAGYR,MXTECH) ! fraction retrofitted for all model years' and tech types' populations
      real*4       exunitsretrobmy(MXAGYR,MXTECH) ! number of units retrofitted for all model years' and tech types' populations
c
c   --- evap tech ---
c
      ...
      real*4       evemsbmy(MXAGYR,MXEVTECH,MXPOL)
      real*4       evpopbmy(MXAGYR,MXEVTECH)
      real*4       evactbmy(MXAGYR,MXEVTECH)
      real*4       evfulbmy(MXAGYR,MXEVTECH)
      real*4       fulbmytot
      real*4       evpoptot
      real*4       evacttot
      real*4       evstrtot
...
c
      ...
      hpval = avghpc(icurec)
c
c   --- set the loop indexes for number of counties ---
c
      if(idxsta .GT. 0 .AND. lstacd(idxsta))then
         ibegj = idxcty(idxsta)
         iendj = idxcty(idxsta+1) - 1
      else
         ibegj = 1
         iendj = NCNTY
      endif
...
c
      ...
      if( popsta .LE. 0. ) then
         do i = ibegj, iendj
            if( lfipcd(i) ) then
               call wrtdat(jerr,fipcod(i),subcur,asccod,hplev,
...
c       county based on state to county allocation ---
c
      call alocty(jerr,popcty,grwcty,icurec,
     &      popsta,idxsta,idxasc,luse,growth)
      if( jerr .NE. ISUCES ) goto 9999
c
...
c-----------------------------------------------------------------------
c
      call daymthf(asccod, fipsta, daymthfac, mthf, dayf, ndays)
c
...
c
     ...
     &         'State','SCC','HP range',
     &         fipsta,asccod,hprang(1,icurec),hprang(2,icurec)
...
c
      call grwfac( jerr, grwsta, ipopyr(icurec),
     &      ipopyr(icurec)+1, fipsta, indgrx(idxgrw) )
      if( jerr .NE. ISUCES ) goto 9999
      grwcty = grwsta
c
...
c
     ...
     &      faclod(idxact), actage(idxact), grwsta )
c
...
c
      call agedist( jerr, popsta, modfrc, ipopyr(icurec),
     &      igryr, nyrlif, yryrfrcscrp, fipsta, indgrx(idxgrw) )
      if( jerr .NE. ISUCES ) goto 9999
c
...
c
            popbmy = popsta * modfrc(idxyr) * tchfrc(idxtch(idxyr),i)
            expopbmy(idxyr,i) = popbmy
c
...
c
     ...
     &               popbmy, asccod, hpval, iyr, tecnam(i) )
               if( jerr .NE. ISUCES ) goto 9999
               exfracretrobmy(idxyr,i) = fracretrobmy
               exunitsretrobmy(idxyr,i) = unitsretrobmy
...
c
     ...
     &            hpval, denful, bsfc(idxyr,i), idxact, adjems, adjtime,
     &            emsfac, tpltmp, stradj(idxyr), popsta, modfrc(idxyr),
     &            ndays, actadj(idxyr) )
            if( jerr .NE. ISUCES ) goto 9999
            do j = 1, MXPOL
               exemsbmy(idxyr,i,j) = emsbmy(j)
            end do
c
...
c
     ...
     &            * tplful * tchfrc(idxtch(idxyr),i) * adjtime
            exactbmy(idxyr,i) = actbmy
            fulbmy = tplful * popsta * actadj(idxyr) * modfrc(idxyr)
     &            * tchfrc(idxtch(idxyr),i) * (hpval * faclod(idxact)
     &            * bsfc(idxyr,i) / denful ) * adjtime
            exfulbmy(idxyr,i) = fulbmy
c
...
c
         ...
         acttot = acttot + actadj(idxyr) * popsta
     &         * modfrc(idxyr) * tplful * adjtime
         strtot = strtot + stradj(idxyr) * popsta
     &         * modfrc(idxyr) * tplful * adjtime
c
c   --------- Evap --------
c
...
c
     ...
     &            sre10fac(i), vnte10fac(i), fipsta )
            if( jerr .NE. ISUCES ) goto 9999
            do j = 1, MXPOL
               evemsbmy(idxyr,i,j) = emsbmy(j)
            end do
c
c   --- calculate the contribution from this tech type ---
c
            popbmy = popsta * modfrc(idxyr)
     &            * evtchfrc(idxevtch(idxyr),i)
            evpopbmy(idxyr,i) = popbmy
            actbmy = actadj(idxyr) * popsta * modfrc(idxyr)
     &            * tplful * evtchfrc(idxevtch(idxyr),i) * adjtime
            evactbmy(idxyr,i) = actbmy
c
...
c   --- sum population, activity and starts ---
c
         evpoptot = evpoptot + popsta * modfrc(idxyr)
         evacttot = evacttot + actadj(idxyr) * popsta
     &         * modfrc(idxyr) * tplful * adjtime
         evstrtot = evstrtot + stradj(idxyr) * popsta
     &         * modfrc(idxyr) * tplful * adjtime
c
c   --- next model year ---
c
...
c
c   -----------------------------------------------------------------
c   --- allocate the state-level calculations to the county level ---
c   -----------------------------------------------------------------
c
c   --- loop over counties ---
c
      do idxfip = ibegj, iendj
c
c   --- if the county was not requested, skip to the next county ---
c
         if( .NOT. lfipcd(idxfip) ) then
            cycle
         endif
c
c   --- if the population for this county is zero, just write
c       a zero record and skip to the next county ---
c
         if( popcty(idxfip) .LE. 0 ) then
            call wrtdat(ierr,fipcod(idxfip),subcur,asccod,hplev,
     &            0.,0.,0.,0.,0.,0.,0.,zeroems)
...
c
c   --- calculate population fraction for the county ---
c
         popctyfrac = popcty(idxfip) / popsta
c
c   --- loop over model years ---
c
         do iyr = iepyr - nyrlif + 1, iepyr
...
c
            idxyr = iepyr - iyr + 1
c
c   --- if the fraction for this model year is zero (sometimes
c       happens for the first year in the distribution), skip to
c       the next model year ---
c
            if( modfrc(idxyr) .LE. 0. ) then
               cycle
            endif
c
c           CIM_NOTDONE: Convert *bmy variables (popbmy, emsbmy, etc.) used below
c                        to have a model-year dimension; this will require modifying
c                        the code in the state-level processing to populate them
c                        correctly as well
c
c   --- loop over exhaust tech types ---
c
            do i= 1, ntech(idxtch(idxyr))
c
c   --- skip over unused technology types ---
...
c
c   --- allocate the exhaust by-model-year state-level
c       values to the county level ---
c
               popbmy = expopbmy(idxyr,i) * popctyfrac
               actbmy = exactbmy(idxyr,i) * popctyfrac
               fulbmy = exfulbmy(idxyr,i) * popctyfrac
               fracretrobmy = exfracretrobmy(idxyr,i) ! fraction retrofitted same for state and county level
               unitsretrobmy = exunitsretrobmy(idxyr,i) * popctyfrac
               do j = 1, MXPOL
                  emsbmy(j) = exemsbmy(idxyr,i,j) * popctyfrac
               end do
c
c   --- write exhaust by-model-year output ---
c
               if( lbmyfl ) then
                  call wrtbmy(jerr,fipcod(idxfip),subcur,asccod,hplev,
     &                  tectyp(idxtch(idxyr),i),iyr,popbmy,emsbmy,
     &                  fulbmy,actbmy,faclod(idxact),hpval,fracretrobmy,
     &                  unitsretrobmy,1) ! 1=exhaust
                  if( jerr .NE. ISUCES ) goto 9999
               endif
c
c   --- add to totals for SI report ---
c
               if( lsifl ) then
...
c
c   --- next exhaust tech type ---
c
            end do
c
c   --- loop over evap tech types ---
c
            do i= 1, nevtech(idxevtch(idxyr))
c
c   --- skip over unused technology types ---
...
c
c   --- allocate the evap by-model-year state-level
c       values to the county level ---
c
               popbmy = evpopbmy(idxyr,i) * popctyfrac
               actbmy = evactbmy(idxyr,i) * popctyfrac
               fulbmy = evfulbmy(idxyr,i) * popctyfrac
               do j = 1, MXPOL
                  emsbmy(j) = evemsbmy(idxyr,i,j) * popctyfrac
               end do
c
c   --- write evap by-model-year output ---
c
               if ( levbmyfl )  then
...
c
c   --- allocate the total state-level values to the county level ---
c
         poptotcty = poptot * popctyfrac
         acttotcty = acttot * popctyfrac
         fulcsmcty = fulcsm * popctyfrac
         fracretrocty = fracretro ! fraction retrofitted same for state and county level
         unitsretrocty = unitsretro * popctyfrac
         do i = 1, MXPOL
            emsdaycty(i) = emsday(i) * popctyfrac
         end do
c
c   --- write the data to the output data file ---
c
         call wrtdat(jerr,fipcod(idxfip),subcur,asccod,hplev,
     &         poptotcty,acttotcty,fulcsmcty,faclod(idxact),hpval,
     &         fracretrocty,unitsretrocty,emsdaycty)
         if( jerr .NE. ISUCES ) then
            goto 9999
         endif
c
...
c
         do i = 1, MXPOL
            if( emsdaycty(i) .GT. 0. ) then
               emsams(idxfip,i) = emsams(idxfip,i) + emsdaycty(i)
            endif
         end do
         namsrc = namsrc + 1
c
